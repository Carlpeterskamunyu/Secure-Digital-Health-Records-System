...
<head>
    <title>Secure Digital Health Records System</title>
    <meta charset="utf-8" />
...
<body>
    <div class="navbar navbar-default navbar-fixed-top">
        ...
        <label class="navbar-brand">Secure Digital Health Records System</label>
        <?php
    ...
    <div id="sidebar">
        <ul id="menu" class="nav menu">
            ...
            <li>
                <a href="#"><i class="glyphicon glyphicon-folder-close"></i> Sections</a>
                <ul>
    ...
    <div id="content">
        ...
        <div style="display:none;" id="add_itr" class="panel panel-success">
            <div class="panel-heading">
                ...
                <button id="hide_itr" style="float:right; margin-top:-4px;" class="btn btn-sm btn-danger"><span class="glyphicon glyphicon-remove"></span> CLOSE
                </button>
            </div>
            <div class="panel-body">
                <form id="form_dental" method="POST" enctype="multipart/form-data">
                    ...
                    <br />
                    <div class="form-inline">
                        <button class="btn btn-primary" name="save_patient"><span class="glyphicon glyphicon-save"></span> SAVE
                        </button>
                    </div>
                </form>
            </div>
                ...
                <table id="table" class="display" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        ...
                        while ($fetch = $query->fetch_array()) {
                            ...
                            $qrCode = QRCode::create("ITR No: {$fetch['itr_no']}\nName: {$fetch['firstname']} {$fetch['lastname']}]\nBirthdate: {$fetch['birthdate']}\nAge: {$fetch['age']}\nAddress: {$fetch['address']}\nCivil Status: {$fetch['civil_status']}")
                            ...
                            echo '<tr>';
                            echo '<td>' . $fetch['itr_no'] . '</td>';
                            echo '<td>' . $fetch['firstname'] . ' ' . $fetch['lastname'] . '</td>';
                            echo '<td>' . $fetch['birthdate'] . '</td>';
                            echo '<td>' . $fetch['age'] . '</td>';
                            echo '<td>' . $fetch['address'] . '</td>';
                            echo '<td>' . $fetch['civil_status'] . '</td>';
                            echo '<td><center>';
                            echo '<a href="complaints.php?id=' . $fetch['itr_no'] . '&lastname=' . $fetch['lastname'] . '" class="btn btn-sm btn-info">Complaints</a>';

                            // Display the QR code
                            echo '<span class="badge"><img src="data:image/png;base64,' . base64_encode($qrCodeString) . '" alt="QR Code" style="height: 100px"; margin="2px";/></span> ';
                            
                            echo '<a href="edit_patient.php?id=' . $fetch['itr_no'] . '&lastname=' . $fetch['lastname'] . '" class="btn btn-sm btn-warning"><span class="glyphicon glyphicon-pencil"></span> Update</a>';
                            echo '</center></td>';
                            echo '</tr>';
                        }
                        $conn->close();
